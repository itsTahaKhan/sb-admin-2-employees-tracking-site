...
if($action==='fetchMonthlyActive'){
    ...
    if ($search !== "") {
        $where = "WHERE (e.fname LIKE '%$search%' 
                  ...
                  AND
                  (ud.last_activity >= CURDATE() - INTERVAL 29 DAY)
                ";
    }
    else{
        $where = "WHERE (ud.last_activity >= CURDATE() - INTERVAL 29 DAY)
                ";
    }
    ...
    $totalRes = $conn->query("
                                ...
                                WHERE ud.last_activity >= CURDATE() - INTERVAL 29 DAY
                            ");
...
if($action==='fetchMonthlyInactive'){
    $start  = $_POST['start'] ?? 0;      
    $length = $_POST['length'] ?? 10;    
    $search = $_POST['search']['value'] ?? '';
...
if($action==='fetchDailyActive'){
    ...
    if ($search !== "") {
        $where = "WHERE (e.fname LIKE '%$search%' 
                  ...
                  OR ed.design_name LIKE'%$search%')
                  AND
                  (DATE(ud.last_activity) = CURDATE())
                ";
    }
    else{
        $where = "WHERE (DATE(ud.last_activity) = CURDATE())
                ";
    }
    ...
    $totalRes = $conn->query("
                                ...
                                FROM employees e
                                LEFT JOIN userdata ud ON e.emp_id = ud.emp_id
                                WHERE DATE(ud.last_activity) = CURDATE()
                            ");
